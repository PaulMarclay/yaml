...
class Yaml_SfYamlParser
{
    ...
    while ($this->moveToNextLine())
    {
      ...
      if (preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+?))?\s*$#u', $this->currentLine, $values))
      {
        ...
        if (!isset($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#'))
        {
          ...
          $parser = new Yaml_SfYamlParser($c);
        ...
        else
        {
          if (isset($values['leadspaces'])
            && ' ' == $values['leadspaces']
            && preg_match('#^(?P<key>'.Yaml_SfYamlInline::REGEX_QUOTED_STRING.'|[^ \'"\{].*?) *\:(\s+(?P<value>.+?))?\s*$#u', $values['value'], $matches))
          {
      ...
      else if (preg_match('#^(?P<key>'.Yaml_SfYamlInline::REGEX_QUOTED_STRING.'|[^ \'"].*?) *\:(\s+(?P<value>.+?))?\s*$#u', $this->currentLine, $values))
      {
        $key = Yaml_SfYamlInline::parseScalar($values['key']);
      ...
      else
      {
        ...
        {
          $value = Yaml_SfYamlInline::load($this->lines[0]);
          if (is_array($value))
          {
            $first = reset($value);
            if ('*' === substr($first, 0, 1))
  ...
  protected function parseValue($value)
  {
    ...
    {
      return Yaml_SfYamlInline::load($value);
    }
  }
